...
namespace Gamekit2D
{
    public class HubDoor : MonoBehaviour, IDataPersister
    {
        ...
        void CheckInventory()
        {
            ...
            if (stateIndex >= 0)
            {
                if(keyDirectorTrigger) keyDirectorTrigger.OverrideAlreadyTriggered (true);
                if(m_SpriteRenderer) m_SpriteRenderer.sprite = unlockStateSprites[stateIndex];
        ...
        public void LoadData(Data data)
        {
            ...
            if(m_SpriteRenderer) m_SpriteRenderer.sprite = d.value;
        }
